"""
剪切环工况 —— 随机剪切环(滑移环)网络,无预处理,直接正式加载
(预弛豫 否 / 预变形 否 / 二次弛豫 否 / 正式加载 是)

初始构型从 init_shear_seed<seed>/init_config.data 读取
(先运行 generate_shear_loops.py 生成该文件)。

剪切环受载即在 {110} 面内膨胀承载塑性剪切,故跳过弛豫直接加载
(无载弛豫下小环会因线张力自湮灭)。写法参考 caseA_baseline.py,
...
# ⚠️ Fe BCC 参数 —— 占位值,待用户最终敲定(与 caseA_baseline.py 一致)
state = {
    "crystal":   'bcc',
    ...
    "nu":        0.29,       # 泊松比 ⚠️待定
    "a":         4.0,        # 核心参数
    "maxseg":    600,      # ≈50nm,与生成脚本 MAXSEG_B 一致
    "minseg":    150,        # ≈12nm
    "rtol":      1.0,
    "rann":      2.0,
    "nextdt":    1e-9,
...
def main():
    ...
    parser.add_argument('--seed', type=int, default=12345, help='随机实现种子(对应 init_shear 目录)')
    parser.add_argument('--restart', type=int, help='从指定步骤重启')
    args = parser.parse_args()

    base_dir   = os.path.dirname(os.path.abspath(__file__))
    init_dir   = os.path.join(base_dir, f'init_shear_seed{args.seed}')
    output_dir = os.path.join(base_dir, f'output_caseShear_seed{args.seed}')
