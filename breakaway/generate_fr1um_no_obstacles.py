"""
生成 5 um 立方盒、1 um Frank-Read 源、rho0≈1e12 m^-2 的无障碍初始网络。
...
MAXSEG_B = 160.0
FOREST_PER_SYS = 2
PLACEMENT_EXTRA_B = 320.0  # 与点障碍组的300b面内偏移+20b离面距离一致
...
def insert_fr_loop(nodes, segs, burg, plane, arm_len_b, center):
    """插入一个闭合 FR 回路；活动臂可动，另外三条虚拟闭合臂钉扎。"""
...
def build_network(rng):
    ...
    margin = arm_b + PLACEMENT_EXTRA_B
    nodes, segs = [], []
    for isys, count in enumerate(counts):
        for _ in range(count):
            center = rng.uniform(margin, Lbox_b - margin, size=3)
            insert_fr_loop(nodes, segs, bsys[isys], nsys[isys], arm_b, center)
...
def main():
    ...
    try:
        ...
        out_dir = os.path.join(base_dir, 'init_fr1um_no_obstacles')
