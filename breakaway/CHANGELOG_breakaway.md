...
- **脱钉后尖角"一步愈合"排疑(2026-07,导师质疑)**:逐帧追同一节点(NodeTag 不变)三帧:constraint 7→0(释放)→ 位置落回线上;节点全程保留。证明尖角消失是**物理弛豫**(尖处回复力大、一个 dt 拉平),**不是 remesh 删节点**——扫过面积正常记账,无塑性应变漏洞。判别法备忘:分辨"弹回去 vs 被删掉"只能追节点身份(NodeTag),看渲染分不出。
