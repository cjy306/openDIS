...
- **[5a] 索引重排崩溃修复(首版 ~6h 段错误,已修)**:首版按"索引"读 xold(仅防了追加越界),
  但 retroactive 碰撞在真实合并的步会 `purge_network()`→`remove_nodes` **末尾填空位重排索引**
  (collision_retroactive.cpp:1839;network.cpp:397)→ 按索引读到**别的节点**的旧位置 → 垃圾扫掠
  四边形 → 误捕获+τ回退瞬移真实节点 → 拓扑错乱 → 段错误。首次真碰撞才触发重排,故 6h 才炸;
  旧 v 反推免疫(读当前索引的当前量,永远自洽)。retroactive 自家用 xold 安全是因为它在**自己
  合并之前**读——安全性取决于"用的时机",教训记下。
  **修复 = 方案1+2 叠加**:① handle() 入口(retroactive 之前、索引尚一致时)建 `tag→步初位置`
  unordered_map(tag 随节点结构体搬家,重排后仍唯一),Phase A 按 tag 查表,查不到(当步新生)
  退回 v 反推;② 位移上限保险丝:|p−o| > 2·maxseg 视为条目失效退回 v 反推(防未知重排源)。
  无障碍时建表直接 return,零开销约定保持。
  - **认知状态(2026-07-11 定格)**:根因链每环代码查实、时序吻合,但崩溃 run 日志被删,
    死亡特征(崩溃前 captured 暴涨+节点尖峰)未核对 → **"高度可能、未获直接证实"**。
    修复正当性不受影响(漏洞代码层实存)。junction 指纹=0 只排除异号线碰撞,**排除不了
    同线自碰湮灭**(b−b=0 无 junction,仍触发 purge 重排)→ "[5a] 是否已被实测行使"开案,
    将来 run 若再现"captured 暴涨+节点尖峰+段错误"三联征即同案重启。
  - **修复后 field run 万步验证(2026-07-11)**:V1 40/40 捕获、V2 钉点 max 0.319b、
    V3 漂移 max 0.252b、无段错误;释放角分布主体 82-90°(平稳期滞后 ~5°,深滞后
    11-56° 集中在 ~9000 步雪崩窗口);49 次同障碍重捕=抖动疑似回归(自限、待量化);
    V4 判据对 field 过严待改。
