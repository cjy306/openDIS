...
## 验证进展(2026-07-13,首炉判读)
- 首炉(默认 A=1.02e11, Rp=10nm, tau=50MPa, 500nm 盒):位错近颗粒**轻微弓出后切过**。
- **判读:这是正确物理,非 bug**。经 PMC 原文复核(公式/A/Rp/节点力与实现逐项一致):
  F_max≈0.86A/Rp≈1.3e-10 N << μb²≈5e-9 N,该参数本质是**中等偏弱可切过障碍**;
  Friedel τ_c≈F_tot/(bL),L=500nm(PBC)→ **τ_c≈2–8 MPa**,50 MPa 十倍碾过。
  Lehtinen"strong"的语境是其高密度(L=30–70nm),τ_c∝1/L。A 换算 1.02e11 证实无误。
- **下一步(取代原 A 扫描)**:同构型同 A,tau=2/5/10/20 MPa 四炉测过/不过分界;
  分界落在 2–8 MPa 窗 → 第 3+4 级闭合,且为第 5 级 BKS/Friedel 对标首点。
- ⚠️ 实现特性(原文同款,记入方法学):节点力方案总阻力 ∝ 1/节点间距,
  障碍强度随颗粒附近离散细化而变;生产与验证的 minseg/maxseg 需保持一致。
